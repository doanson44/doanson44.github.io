# Light Mode Design Rules â€” doanson44.github.io
...
Light mode can easily become "gray on gray" â€” ensure body text has at least 4.5:1 contrast (WCAG AA). Bootstrap light mode provides this by default.
...
## Platform Custom Variables â€” Light Mode
...
|---------|-------------------|
| Toolbar buttons | `btn-outline-secondary` â€” visible border on light bg |
...
| Theme toggle icon | `bi-sun-fill` shown (ðŸŒ™ hidden), hover color: `--ms-accent-orange` |
...
The h1 gradient uses `background-clip: text` with transparent fill â€” this breaks in light mode if colors are too bright. Ensure light mode gradients use darker shades:
...
- No need to change Mermaid config between themes â€” transparent SVG adapts automatically
...
- [ ] Toggle icon switches correctly (â˜€ï¸â†”ðŸŒ™)
