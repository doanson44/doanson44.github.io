...
pub fn json_diff(left: &str, right: &str) -> Result<String, String> {
    ...
    if changes.is_empty() {
        Ok("No structural differences.".into())
    } else {
        Ok(changes.join("\n"))
    }
}
        ...
        (Value::Object(ao), Value::Object(bo)) => {
            for key in ao.keys() {
                if !bo.contains_key(key) {
                    changes.push(format!("Removed {path}.{key}"));
                }
            }
            for key in bo.keys() {
                if !ao.contains_key(key) {
                    changes.push(format!("Added {path}.{key}: {}", bo[key]));
                }
            }
            for key in ao.keys().filter(|k| bo.contains_key(*k)) {
                diff_value(&format!("{path}.{key}"), &ao[key], &bo[key], changes);
            }
        }
        (Value::Array(aa), Value::Array(ba)) => {
            ...
            for i in 0..max {
                match (aa.get(i), ba.get(i)) {
                    (Some(x), Some(y)) => diff_value(&format!("{path}[{i}]"), x, y, changes),
                    (Some(_), None) => changes.push(format!("Removed {path}[{i}]")),
                    (None, Some(y)) => changes.push(format!("Added {path}[{i}]: {y}")),
                    _ => {}
                }
            }
        }
...
pub fn json_path(source: &str, expression: &str) -> Result<String, String> {
        ...
        .filter(|s| !s.is_empty())
    {
        if let Some((name, index)) = token.split_once('[') {
            current = current
                .get(name)
                .ok_or_else(|| format!("Path not found: {name}"))?;
            let index = index
                .trim_end_matches(']')
                .parse::<usize>()
            ...
            return Err("Invalid JSONPath token.".into());
        } else {
            current = current
                .get(token)
...
pub fn json_to_type(source: &str, target: &str) -> Result<String, String> {
    ...
    let root = value
        .as_object()
        .ok_or("Root JSON value must be an object.")?;
...
fn rust_type(v: &Value) -> &'static str {
    match v {
        Value::String(_) => "String",
        Value::Bool(_) => "bool",
        Value::Number(n) if n.is_i64() => "i64",
        Value::Number(_) => "f64",
        Value::Array(_) => "Vec<serde_json::Value>",
        _ => "serde_json::Value",
    }
}
fn csharp_type(v: &Value) -> &'static str {
    match v {
        Value::String(_) => "string",
        Value::Bool(_) => "bool",
...
fn rust_struct(name: &str, obj: &serde_json::Map<String, Value>) -> String {
    let mut out =
        format!("#[derive(Debug, serde::Serialize, serde::Deserialize)]\npub struct {name} {{\n");
    for (k, v) in obj {
        out.push_str(&format!(
            "    pub {}: {},\n",
            k.replace('-', "_"),
...
fn csharp_class(name: &str, obj: &serde_json::Map<String, Value>) -> String {
    let mut out = format!("public sealed class {name}\n{{\n");
    for (k, v) in obj {
        out.push_str(&format!(
            "    public {} {} {{ get; set; }}\n",
            csharp_type(v),
...
fn ts_interface(name: &str, obj: &serde_json::Map<String, Value>) -> String {
...
fn go_struct(name: &str, obj: &serde_json::Map<String, Value>) -> String {
...
fn python_dataclass(name: &str, obj: &serde_json::Map<String, Value>) -> String {
...
pub fn openapi(source: &str) -> Result<String, String> {
    let value = if let Ok(json) = serde_json::from_str::<Value>(source) {
        ...
        let yaml: serde_yaml::Value =
            serde_yaml::from_str(source).map_err(|e| format!("Invalid OpenAPI JSON/YAML: {e}"))?;
        serde_json::to_value(yaml)
            .map_err(|e| format!("Unable to normalize OpenAPI document: {e}"))?
    };
    let version = value
        .get("openapi")
        .and_then(Value::as_str)
        .ok_or("OpenAPI document is missing the openapi version.")?;
    let title = value
        .pointer("/info/title")
        .and_then(Value::as_str)
...
pub fn sql_to_entity(source: &str, target: &str) -> Result<String, String> {
    ...
    let start = lower
        .find("create table")
        .ok_or("Expected CREATE TABLE statement.")?;
    ...
    let table = head
        ...
        .trim_matches('`');
    let columns = split_sql_columns(&source[open + 1..close])
        .into_iter()
        .filter_map(|part| {
            ...
            Some((
                ...
                sql_type.to_ascii_uppercase(),
            ))
        })
        .collect::<Vec<_>>();
    if columns.is_empty() {
        return Err("No columns found in CREATE TABLE statement.".into());
    }
    ...
    if target == "c#" || target == "csharp" {
        ...
        Err("Supported targets: Rust, C#, CSharp.".into())
    }
}
...
fn rust_sql_type(ty: &str) -> &'static str {
    if ty.contains("INT") {
        "i64"
    } else if ty.contains("BOOL") {
        "bool"
    } else if ty.contains("DEC") || ty.contains("NUM") || ty.contains("FLOAT") {
        "f64"
    } else {
        "String"
    }
}
fn csharp_sql_type(ty: &str) -> &'static str {
    if ty.contains("INT") {
        "long"
    } else if ty.contains("BOOL") {
        "bool"
