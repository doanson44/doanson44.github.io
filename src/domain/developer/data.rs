...
pub fn json_diff(left: &str, right: &str) -> Result<String, String> {
    ...
    let mut changes = Vec::new(); diff_value("$", &a, &b, &mut changes);
...
fn diff_value(path: &str, a: &Value, b: &Value, changes: &mut Vec<String>) {
    match (a, b) {
        ...
        (Value::Array(aa), Value::Array(ba)) => { let max = aa.len().max(ba.len()); for i in 0..max { match (aa.get(i), ba.get(i)) { (Some(x), Some(y)) => diff_value(&format!("{path}[{i}]"), x, y, changes), (Some(_), None) => changes.push(format!("Removed {path}[{i}]")), (None, Some(y)) => changes.push(format!("Added {path}[{i}]: {y}")), _ => {} } } }
        _ if a != b => changes.push(format!("Changed {path}: {a} → {b}")), _ => {}
    }
}
...
pub fn json_path(source: &str, expression: &str) -> Result<String, String> {
    ...
    let expr = expression.trim(); if !expr.starts_with('$') { return Err("JSONPath must start with $.".into()); }
    let mut current = &value;
    for token in expr.trim_start_matches('$').split('.') .filter(|s| !s.is_empty()) {
        if let Some((name, index)) = token.split_once('[') { current = current.get(name).ok_or_else(|| format!("Path not found: {name}"))?; let index = index.trim_end_matches(']').parse::<usize>().map_err(|_| "Invalid array index.")?; current = current.get(index).ok_or("Array index not found.")?; }
        else if token.contains('[') { return Err("Invalid JSONPath token.".into()); } else { current = current.get(token).ok_or_else(|| format!("Path not found: {token}"))?; }
    }
...
fn type_name(key: &str) -> String { key.split(['_', '-', ' ']).filter(|s| !s.is_empty()).map(|s| { let mut c = s.chars(); match c.next() { Some(first) => first.to_uppercase().collect::<String>() + c.as_str(), None => String::new() } }).collect() }
...
pub fn openapi(source: &str) -> Result<String, String> {
    let value: Value = serde_json::from_str(source).map_err(|e| format!("OpenAPI viewer currently accepts JSON: {e}"))?;
    let title = value.pointer("/info/title").and_then(Value::as_str).unwrap_or("Untitled API"); let version = value.pointer("/info/version").and_then(Value::as_str).unwrap_or("unknown");
    let mut out = format!("{title}\nVersion: {version}\n\nEndpoints:\n");
    if let Some(paths) = value.get("paths").and_then(Value::as_object) { for (path, methods) in paths { if let Some(methods) = methods.as_object() { for (method, op) in methods { let summary = op.get("summary").and_then(Value::as_str).or_else(|| op.get("description").and_then(Value::as_str)).unwrap_or(""); out.push_str(&format!("{} {} {}\n", method.to_uppercase(), path, summary)); } } } }
    Ok(out.trim_end().into())
}

pub fn sql_to_entity(source: &str, target: &str) -> Result<String, String> {
    let lower = source.to_ascii_lowercase(); let start = lower.find("create table").ok_or("Expected CREATE TABLE statement.")?; let open = source[start..].find('(').ok_or("Missing column list.")? + start; let close = source.rfind(')').ok_or("Missing closing parenthesis.")?; let head = &source[start..open]; let table = head.split_whitespace().last().ok_or("Missing table name.")?.trim_matches('`');
    let columns = source[open+1..close].split(',').filter_map(|part| { let mut p = part.split_whitespace(); let name = p.next()?; let sql_type = p.next()?; Some((name.trim_matches('`').to_string(), sql_type.to_ascii_uppercase())) }).collect::<Vec<_>>();
    let target = target.to_ascii_lowercase(); if target == "c#" || target == "csharp" { let mut out = format!("public sealed class {}\n{{\n", type_name(table)); for (name, ty) in columns { out.push_str(&format!("    public {} {} {{ get; set; }}\n", csharp_sql_type(&ty), type_name(&name))); } out.push('}'); Ok(out) } else { let mut out = format!("pub struct {} {{\n", type_name(table)); for (name, ty) in columns { out.push_str(&format!("    pub {}: {},\n", name, rust_sql_type(&ty))); } out.push('}'); Ok(out) }
}
