...
pub fn http_status(input: &str) -> Result<String, String> {
    let code: u16 = input
        .trim()
        .parse()
        .map_err(|_| "Enter an HTTP status code.")?;
    let text = match code {
        100 => "Continue",
...
pub fn subnet(input: &str) -> Result<String, String> {
    let (ip, prefix) = input
        .trim()
        .split_once('/')
        .ok_or("Use CIDR notation such as 192.168.1.0/24.")?;
...
pub fn curl(input: &str) -> Result<String, String> {
    ...
    let first = lines.next().ok_or("Enter a request line.")?;
    let mut parts = first.split_whitespace();
    let method = parts.next().unwrap_or("GET").to_uppercase();
    let url = parts
        .next()
        .ok_or("Request line must contain METHOD URL.")?;
...
pub fn headers(input: &str) -> Result<String, String> {
    ...
    for line in input.lines() {
        if let Some((name, value)) = line.split_once(':') {
            let name = name.trim();
            let note = match name.to_ascii_lowercase().as_str() {
                "content-type" => "Describes the representation format.",
                "cache-control" => "Controls caching behavior.",
                "authorization" => "Carries authentication credentials; treat values as secrets.",
...
pub fn mime(input: &str) -> Result<String, String> {
    ...
    if let Some((ext, mime)) = table
        .iter()
        .find(|(ext, mime)| *ext == value || *mime == input.trim())
    {
        Ok(format!("Extension: .{ext}\nMIME type: {mime}"))
    } else {
        Err("MIME type not found in the built-in reference table.".into())
    }
}
