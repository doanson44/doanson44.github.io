...
pub fn sample(kind: ToolKind) -> (&'static str, &'static str) {
    match kind {
        ToolKind::Xml => (
            "<root><user><name>Son</name><active>true</active></user></root>",
            "",
        ),
        ToolKind::Yaml => (
        ...
        ToolKind::Html => (
            "<main><section><h1>Hello</h1><p>Developer tools</p></section></main>",
            "",
        ),
        ToolKind::Css => (
            ".card{display:flex;gap:1rem;padding:1rem}.card:hover{opacity:.9}",
            "",
        ),
        ToolKind::Javascript => (
        ...
        ToolKind::Regex => (
            r"\b[A-Z][a-z]+\b",
            "Son writes Rust. ChatGPT helps Son build tools.",
        ),
    ...
    for ch in source.chars() {
        if ch == '<' {
            if !token.trim().is_empty() {
                write_line(&mut out, depth, token.trim());
            }
            token.clear();
            token.push(ch);
        } else if ch == '>' {
        ...
        match ch {
            '{' => {
                if !current.trim().is_empty() {
                    write_line(&mut out, depth, current.trim());
                }
                current.clear();
                write_line(&mut out, depth, "{");
                depth += 1;
            }
            '}' => {
                if !current.trim().is_empty() {
...
fn convert_timestamp(input: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "UTC: {year:04}-{month:02}-{day:02} {:02}:{:02}:{:02}Z",
        seconds / 3600,
        (seconds % 3600) / 60,
        seconds % 60
    ))
...
fn convert_color(input: &str) -> Result<String, String> {
    ...
    let r = u8::from_str_radix(&hex_value[0..2], 16).map_err(|_| "Invalid hexadecimal color.")?
        as f64
        / 255.0;
    let g = u8::from_str_radix(&hex_value[2..4], 16).map_err(|_| "Invalid hexadecimal color.")?
        as f64
        / 255.0;
    let b = u8::from_str_radix(&hex_value[4..6], 16).map_err(|_| "Invalid hexadecimal color.")?
        as f64
        / 255.0;
...
fn validate_cron(input: &str) -> Result<String, String> {
    ...
    if fields.len() != 5 {
        return Err("A standard cron expression must contain exactly five fields.".into());
    }
    ...
    for i in 0..5 {
        ...
        {
            return Err(format!("Invalid {} field.", names[i]));
        }
        writeln!(&mut out, "{}: {} ({:?})", names[i], fields[i], ranges[i]).unwrap();
    }
    Ok(out.trim_end().into())
}
...
fn lookup_status(input: &str) -> Result<String, String> {
    let code: u16 = input
        .trim()
        .parse()
        .map_err(|_| "Enter an HTTP status code such as 404.")?;
    let text = match code {
        100 => "Continue",
...
fn calculate_subnet(input: &str) -> Result<String, String> {
    let (ip, prefix) = input
        .trim()
        .split_once('/')
        .ok_or("Enter CIDR notation such as 192.168.1.0/24.")?;
...
fn ipv4_to_u32(input: &str) -> Result<u32, String> {
    ...
    let mut value = 0u32;
    for p in parts {
        let n: u8 = p.parse().map_err(|_| "Invalid IPv4 address.")?;
...
fn generate_qr(input: &str) -> Result<String, String> {
    let code = qrcode::QrCode::new(input.as_bytes())
        .map_err(|e| format!("Unable to generate QR code: {e}"))?;
    Ok(code
...
fn replace_ci(source: &str, needle: &str, replacement: &str) -> String {
    let mut out = String::new();
    let mut rest = source;
...
mod tests {
    ...
    fn url_round_trip() {
        ...
        assert_eq!(decode_url("hello%20world%21").unwrap(), value);
    }
    #[test]
    fn subnet_works() {
        assert!(calculate_subnet("192.168.1.10/24")
            .unwrap()
            .contains("192.168.1.0"));
    }
    #[test]
    fn color_works() {
        assert!(convert_color("#ff0000").unwrap().contains("RGB: 255, 0, 0"));
    }
    #[test]
    fn regex_works() {
        assert!(test_regex(r"\d+", "abc 123").unwrap().contains("123"));
    }
}
