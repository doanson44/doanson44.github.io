...
pub fn timestamp_to_millis(value: &str, unit: TimestampUnit) -> Result<f64, String> {
    let value = value.trim().parse::<f64>().map_err(|_| "Enter a valid numeric timestamp.".to_string())?;
    if !value.is_finite() {
...
mod tests {
    ...
    fn converts_seconds_to_milliseconds() {
        assert_eq!(timestamp_to_millis("1700000000", TimestampUnit::Seconds).unwrap(), 1_700_000_000_000.0);
    }

    #[test]
    fn converts_milliseconds_to_seconds() {
        assert_eq!(millis_to_timestamp(1_700_000_000_000.0, TimestampUnit::Seconds), "1700000000");
    }
