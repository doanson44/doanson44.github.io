...
mod tests {
    ...
    fn english_resource_contains_expected_review_cards() {
        ...
        assert_eq!(cards[0].question, "Explain OOP.");
        assert!(cards[0].answer.contains("Abstraction"));
    }
    ...
    fn vietnamese_resource_contains_expected_review_cards() {
        ...
        assert_eq!(cards[0].id, "cv-1");
        assert_eq!(cards[0].title, "OOP");
        assert_eq!(cards[0].question, "Giải thích OOP.");
        assert!(cards[0].answer.contains("Trừu tượng"));
    }
}
