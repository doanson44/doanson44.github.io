...
pub const ENGLISH_CARDS: &[ReviewCard] = &[
    ...
    ReviewCard {
        ...
        answer: "Write once; the caller supplies the type: `List<T>`, `Dictionary<TKey,TValue>`. Avoids `ArrayList` boxing and runtime cast errors.",
    },
    ReviewCard {
        id: "cv-4",
        section: "1 — Foundation / OOP",
        ...
        answer: "`string` is immutable — each change allocates a new object. `StringBuilder` grows a buffer. Use a builder inside loops.",
    },
    ReviewCard {
        id: "cv-5",
        section: "1 — Foundation / OOP",
        ...
        answer: "Prefer `List<T>`. `ArrayList` stores `object`: compiles, can throw at runtime.",
    },
    ReviewCard {
        id: "cv-6",
        section: "1 — Foundation / OOP",
        ...
        answer: "Default: copy the value (or copy the reference for objects). `ref`: the caller’s variable changes; it must already be assigned. `out`: must assign inside the method; the caller need not assign first. The call site must write `ref` / `out`.",
    },
    ReviewCard {
        id: "cv-7",
        section: "1 — Foundation / OOP",
    ...
    ReviewCard {
        ...
        answer: "Belongs to the type, not an instance. One field per AppDomain, on the Managed Heap. Mutable statics are shared state: hard to test and racy.",
    },
    ReviewCard {
        id: "cv-9",
        section: "1 — Foundation / OOP",
        ...
        answer: "One instance for the app: `services.AddSingleton<T>()`, or a private constructor plus `Instance` / `Lazy<T>`. Never register `DbContext` as singleton. **Captive dependency:** a Singleton that holds a Scoped service (for example `DbContext`) — a production bug.",
    },
    ReviewCard {
        id: "cv-10",
        section: "1 — Foundation / OOP",
        ...
        answer: "A hint against stale reads across threads. Usually the wrong tool: not atomic beyond assignment, does not prevent races. Prefer `Interlocked`, `lock`, concurrent collections. Do not mark `long` or `double`. Narrow valid use: a `_shouldStop` flag so the compiler does not cache it in a register.",
    },
    ReviewCard {
        id: "cv-11",
        section: "1 — Foundation / OOP",
        ...
        answer: "1. Logging / configuration: `ILoggerProvider`, `IConfigurationProvider` — who supplies the implementation.\n2. Observer: `IObservable<T>` — `Subscribe` returns `IDisposable`; push `OnNext` / `OnError` / `OnCompleted`.",
    },
    ReviewCard {
        id: "cv-12",
        section: "1 — Foundation",
    ...
    ReviewCard {
        ...
        answer: "Parse the URL → match a template in the route collection → handler. Example: `{controller=Home}/{action=Index}/{id?}`. MVC’s route handler creates the controller and runs the action.",
    },
    ReviewCard {
        id: "cv-20",
        section: "2 — Backend",
    ...
    ReviewCard {
        ...
        answer: "`Binding` + `DataContext`. Modes: OneWay, TwoWay, OneTime, OneWayToSource. Source should implement `INotifyPropertyChanged`. Lists: `ObservableCollection<T>`.",
    },
    ReviewCard {
        id: "cv-25",
        section: "2 — Backend",
        ...
        answer: "Bundling groups files so the browser makes one request. Minification strips whitespace and comments. Many JS/CSS files = many HTTP requests = slow.",
    },
    ReviewCard {
        id: "cv-26",
        section: "2 — Backend",
        ...
        answer: "**Register:** validate unique email and password policy → hash (ASP.NET Identity / PBKDF2 / bcrypt / Argon2) → insert user → optional email confirm. Never store plaintext. Unique index on email.\n\n**Login:** load user → verify hash → lock out after too many failures → issue cookie or JWT over HTTPS. Use the same error if you must not reveal whether the email exists.",
    },
    ReviewCard {
        id: "cv-27",
        section: "2 — Backend",
    ...
    ReviewCard {
        ...
        title: "C4",
        question: "Explain C4.",
        answer: "Draw **L1 context** (who talks to the system) and **L2 containers** (BFF, API, worker, SQL, Redis, blob). One sequence for the riskiest write. Zoom to components only if asked. If production is one API process, draw **one container with module boxes** — do not pretend you have twelve independently deployed services.\n\n```text\n Staff app ──┐\n Admin web ──┼──► BFF (cookie, Redis session) ──► Azure AD\n             └──► (private JWT)\n                    ├── Identity API\n                    ├── Journey / Timesheet API\n                    └── Campaign / KPI API\n                           │\n                    SQL Server + Redis + Blob\n                    Hangfire worker\n```",
    },
    ReviewCard {
        id: "cv-32",
        section: "3 — Architecture",
    ...
    ReviewCard {
        ...
        answer: "Append-only log, partitions, consumer **offsets**, replay. High throughput. Not an AMQP work queue. Use for streams and audit, not “place order” commands unless you have a real stream platform.",
    },
    ReviewCard {
        id: "cv-39",
        section: "3 — Architecture / Messaging",
        ...
        answer: "In-process (or a dedicated worker) scheduler: fire-and-forget, delayed, recurring cron. Dashboard for succeeded/failed. Examples: nightly KPI, staff day-off, SKU labels, email.\n\nWhen jobs starve HTTP threads, run Hangfire in a **second process** (API only enqueues). Hangfire is not the integration contract for another team — that is a bus plus outbox.",
    },
    ReviewCard {
        id: "cv-40",
        section: "3 — Architecture / Messaging",
        ...
        answer: "Do not hit SQL at 5k/s. Put 5k into a durable queue; drain at 2k/s. Remaining work is processed after the peak. Consumers must be idempotent (limited stock). Add a dead-letter queue. Sync path that also sends mail, points, and rank in the same request will time out — return 200 and queue those side effects.\n\n```text\nSync (bad):  Client → API ──mail──points──rank──► 200 after ~1000 ms\n\nAsync (good): Client → API → 200\n                     └── queue → workers (mail, points, rank)\n\nSpike: Client × 5000/s → Queue → API/DB at 2000/s\n```",
    },
    ReviewCard {
        id: "cv-41",
        section: "3 — Architecture",
    ...
    ReviewCard {
        ...
        answer: "A senior who recites SOLID and “we should use Redis” still fails a TA round. The panel wants **numbers, failure modes, and how you would operate the choice**. This section is that layer.",
    },
    ReviewCard {
        id: "cv-45",
    ...
    ReviewCard {
        ...
        answer: "Severity, commander, comms channel, timeline, “stop the bleeding” before root cause theatre. Rollback is a first-class option. Blameless postmortem: what SLO burned, what fitness function we add so it cannot recur. TA is in the rotation often enough that diagrams meet reality.",
    },
    ReviewCard {
        id: "cv-53",
        section: "3 — Architecture / TA depth the panel will still ask",
    ...
    ReviewCard {
        ...
        answer: "`DbContext` pooling (for example 128) × instance count < SQL max sessions. Redis connections multiplexed (one multiplexer singleton). Blob for photos; SQL holds metadata and a hash. Lifecycle policy to cool storage.",
    },
    ReviewCard {
        id: "cv-56",
        section: "3 — Architecture / TA depth the panel will still ask",
        ...
        answer: "Rewrite the platform. Event-source the timesheet. Twelve microservices with one team. 99.99% SLO on a stack that includes a slow third party. A second ORM “for flexibility”.\n\n---\n\n---",
    },
    ReviewCard {
        id: "cv-57",
        section: "4 — Interview",
    ...
    ReviewCard {
        ...
        answer: "Each question: **Short** (15–30 seconds) then **Deep** (if they follow up). Stay in Short unless they ask “why / how / what if”.",
    },
    ReviewCard {
        id: "cv-60",
...
pub const VIETNAMESE_CARDS: &[ReviewCard] = &[
    ...
    ReviewCard {
        ...
        answer: "Viết một lần, kiểu do caller: `List<T>`, `Dictionary<TKey,TValue>`. Tránh `ArrayList` boxing và lỗi cast runtime.",
    },
    ReviewCard {
        id: "cv-4",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "`string` immutable — mỗi lần đổi là object mới. `StringBuilder` nới buffer. Dùng builder trong vòng lặp.",
    },
    ReviewCard {
        id: "cv-5",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "Ưu tiên `List<T>`. `ArrayList` chứa `object`: compile được, runtime có thể vỡ.",
    },
    ReviewCard {
        id: "cv-6",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "Mặc định copy giá trị (hoặc copy reference với object). `ref`: biến gốc đổi; phải gán trước. `out`: phải gán trong hàm; caller không cần gán trước. Chỗ gọi cũng viết `ref` / `out`.",
    },
    ReviewCard {
        id: "cv-7",
        section: "1 — Nền tảng / OOP",
    ...
    ReviewCard {
        ...
        answer: "Thuộc type, không thuộc instance. Một field / AppDomain trên Managed Heap. Static mutable = state dùng chung, khó test, race.",
    },
    ReviewCard {
        id: "cv-9",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "Một instance: `services.AddSingleton<T>()`, hoặc constructor private + `Instance` / `Lazy<T>`. Không singleton `DbContext`. **Captive dependency:** Singleton giữ service Scoped (ví dụ `DbContext`) — bug production.",
    },
    ReviewCard {
        id: "cv-10",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "Hint chống đọc stale giữa thread. Thường **không** phải công cụ đúng: không atomic ngoài gán, không chống race. Ưu tiên `Interlocked`, `lock`, concurrent collection. Không gắn `long`/`double`. Hẹp: cờ `_shouldStop` để compiler không cache ở register.",
    },
    ReviewCard {
        id: "cv-11",
        section: "1 — Nền tảng / OOP",
        ...
        answer: "1. Logging / configuration: `ILoggerProvider`, `IConfigurationProvider` — ai cung cấp implementation.\n2. Observer: `IObservable<T>` — `Subscribe` trả `IDisposable`; đẩy `OnNext` / `OnError` / `OnCompleted`.",
    },
    ReviewCard {
        id: "cv-12",
        section: "1 — Nền tảng",
    ...
    ReviewCard {
        ...
        answer: "Parse URL → khớp template trong route collection → handler. Ví dụ: `{controller=Home}/{action=Index}/{id?}`. Route handler MVC tạo controller và chạy action.",
    },
    ReviewCard {
        id: "cv-20",
        section: "2 — Backend",
    ...
    ReviewCard {
        ...
        answer: "`Binding` + `DataContext`. Mode: OneWay, TwoWay, OneTime, OneWayToSource. Source nên `INotifyPropertyChanged`. List: `ObservableCollection<T>`.",
    },
    ReviewCard {
        id: "cv-25",
        section: "2 — Backend",
        ...
        answer: "Bundling gom file thành một HTTP request. Minification bỏ khoảng trắng, comment. Nhiều file JS/CSS = nhiều request = chậm.",
    },
    ReviewCard {
        id: "cv-26",
        section: "2 — Backend",
        ...
        answer: "**Register:** validate email unique + password policy → hash (ASP.NET Identity / PBKDF2 / bcrypt / Argon2) → insert user → optional confirm email. Không lưu plaintext. Unique index trên email.\n\n**Login:** load user → verify hash → lockout sau nhiều lần sai → phát cookie hoặc JWT trên HTTPS. Cùng một thông báo lỗi nếu không muốn lộ email có tồn tại.",
    },
    ReviewCard {
        id: "cv-27",
        section: "2 — Backend",
    ...
    ReviewCard {
        ...
        answer: "Vẽ **L1 context** (ai nói với hệ thống) và **L2 container** (BFF, API, worker, SQL, Redis, blob). Một sequence cho write rủi ro nhất. Zoom component chỉ khi được hỏi. Production là một process API thì vẽ **một container với ô module** — đừng giả 12 service deploy độc lập.\n\n```text\n App staff ──┐\n Admin web ──┼──► BFF (cookie, session Redis) ──► Azure AD\n             └──► (JWT nội bộ)\n                    ├── Identity API\n                    ├── Journey / Timesheet API\n                    └── Campaign / KPI API\n                           │\n                    SQL Server + Redis + Blob\n                    Hangfire worker\n```",
    },
    ReviewCard {
        id: "cv-32",
        section: "3 — Kiến trúc",
    ...
    ReviewCard {
        ...
        answer: "Log append, partition, consumer giữ **offset**, replay. Throughput cao. Không phải work queue AMQP. Dùng stream/audit, không phải command “đặt hàng” trừ khi đã có nền tảng stream.",
    },
    ReviewCard {
        id: "cv-39",
        section: "3 — Kiến trúc / Messaging",
        ...
        answer: "Scheduler trong process (hoặc worker riêng): fire-and-forget, delay, cron. Dashboard succeeded/failed. Ví dụ: KPI đêm, day-off, nhãn SKU, email.\n\nKhi job đói thread HTTP, chạy Hangfire **process thứ hai** (API chỉ enqueue). Hangfire không phải contract tích hợp team khác — cái đó là bus + outbox.",
    },
    ReviewCard {
        id: "cv-40",
        section: "3 — Kiến trúc / Messaging",
        ...
        answer: "Đừng đập SQL 5k/s. Đưa 5k vào queue bền; rút 2k/s. Phần còn lại xử sau giờ cao điểm. Consumer phải idempotent (hàng có hạn). Thêm dead-letter. Path sync vừa gửi mail, điểm, rank trong cùng request sẽ timeout — trả 200 và queue các side effect.\n\n```text\nSync (xấu):  Client → API ──mail──điểm──rank──► 200 sau ~1000 ms\n\nAsync (tốt): Client → API → 200\n                     └── queue → worker (mail, điểm, rank)\n\nSpike: Client × 5000/s → Queue → API/DB 2000/s\n```",
    },
    ReviewCard {
        id: "cv-41",
        section: "3 — Kiến trúc",
    ...
    ReviewCard {
        ...
        answer: "Người senior thuộc SOLID và “nên dùng Redis” vẫn trượt vòng TA. Panel muốn **số, failure mode, và cách vận hành lựa chọn**. Phần này là lớp đó.",
    },
    ReviewCard {
        id: "cv-45",
    ...
    ReviewCard {
        ...
        answer: "Severity, commander, kênh comms, timeline, “cầm máu” trước kịch root cause. Rollback là lựa chọn hạng một. Postmortem không đổ lỗi: SLO nào cháy, fitness function nào thêm để không lặp. TA vào on-call đủ để sơ đồ gặp thực tế.",
    },
    ReviewCard {
        id: "cv-53",
        section: "3 — Kiến trúc / Độ sâu TA — panel vẫn hỏi",
    ...
    ReviewCard {
        ...
        answer: "`DbContext` pool (ví dụ 128) × số instance < max session SQL. Redis multiplex (một multiplexer singleton). Blob cho ảnh; SQL giữ metadata và hash. Lifecycle xuống cool storage.",
    },
    ReviewCard {
        id: "cv-56",
        section: "3 — Kiến trúc / Độ sâu TA — panel vẫn hỏi",
        ...
        answer: "Rewrite cả nền tảng. Event-source timesheet. Mười hai microservice một team. SLO 99.99% trên stack có third party chậm. ORM thứ hai “cho linh hoạt”.\n\n---\n\n---",
    },
    ReviewCard {
        id: "cv-57",
        section: "4 — Phỏng vấn",
    ...
    ReviewCard {
        ...
        answer: "Mỗi câu: **Ngắn** (15–30 giây) rồi **Sâu** (khi họ hỏi tiếp). Ở mức Ngắn trừ khi họ hỏi “why / how / nếu”.",
    },
    ReviewCard {
        id: "cv-60",
