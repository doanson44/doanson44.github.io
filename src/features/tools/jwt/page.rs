    ...
    view! {
        <main class="flex flex-1 flex-col overflow-hidden">
            <div class="flex flex-nowrap items-center gap-1 border-b border-[var(--border-color)] p-2" id="jwt-toolbar"><div class="ml-auto flex flex-nowrap gap-1">
                <button type="button" class=button title="Decode JWT" on:click=move |_| state.decode()><span aria-hidden="true">"🔓"</span><span class="ml-1 hidden lg:inline">{move || if i18n.get_locale() == Locale::vi { "Giải mã" } else { "Decode" }}</span></button>
                <button type="button" class=button title="Reset to sample JWT" on:click=move |_| state.reset()><span aria-hidden="true">"↶"</span><span class="ml-1 hidden lg:inline">{move || t!(i18n, common_reset)}</span></button>
                <button type="button" class=button title="Clear JWT" on:click=move |_| state.clear()><span aria-hidden="true">"×"</span><span class="ml-1 hidden lg:inline">{move || t!(i18n, common_clear)}</span></button>
            </div></div>
            <div class="flex items-start gap-2 border-b border-amber-400/40 bg-amber-400/10 px-3 py-2 text-sm text-amber-200" role="note"><span aria-hidden="true">"⚠"</span><span>{move || if i18n.get_locale() == Locale::vi { "Dữ liệu JWT đã giải mã chưa được xác thực bằng mật mã. Không có token nào được gửi lên máy chủ." } else { "Decoded JWT data is not cryptographically verified. No token is sent to a server." }}</span></div>
