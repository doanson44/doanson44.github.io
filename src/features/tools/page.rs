...
fn SearchResults(
    ...
    view! {
        <section aria-live="polite">
            ...
            <div class="row g-3">{if results.is_empty() { view! { <div class="col-12"><div class="alert alert-secondary" role="status">"No tools found. Try another search term."</div></div> }.into_any() } else { results.into_iter().collect_view().into_any() }}</div>
        </section>
    }
}
