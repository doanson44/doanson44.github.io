...
fn SearchResults(
    ...
    for (href, title, description) in general {
        if title.to_lowercase().contains(&query) || description.to_lowercase().contains(&query) {
            results
                .push(view! {<ToolCard href=href title=title description=description/>}.into_any());
        }
    }
...
fn ToolSection(title: &'static str, icon: &'static str, children: Children) -> impl IntoView {
    view! {<section class="mb-5" aria-labelledby=format!("section-{}",title.to_lowercase().replace(' ',"-"))><div class="d-flex align-items-center gap-2 mb-3"><i class=format!("bi {} text-primary",icon) aria-hidden="true"></i><h3 class="h5 mb-0" id=format!("section-{}",title.to_lowercase().replace(' ',"-"))>{title}</h3></div><div class="row g-3">{children()}</div></section>}
}
