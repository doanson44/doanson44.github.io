        ...
        <main class="flex flex-1 flex-col overflow-hidden">
            <div class="flex flex-nowrap items-center gap-1 border-b border-[var(--border-color)] p-2" id="json-toolbar">
                <div class="ml-auto flex flex-nowrap gap-1">
                    ...
                    <button type="button" class=button title="Reset to sample JSON" on:click=move |_| state.reset()><span aria-hidden="true">"↶"</span><span class="ml-1 hidden lg:inline">{move || t!(i18n, common_reset)}</span></button>
                    <button type="button" class=button title="Clear JSON" on:click=move |_| state.clear()><span aria-hidden="true">"×"</span><span class="ml-1 hidden lg:inline">{move || t!(i18n, common_clear)}</span></button>
                </div>
            </div>
            ...
            <ToolSplit initial_ratio=50>
                ...
                <ToolPanel side=ToolPanelSide::Second>
                    <div class="flex h-full flex-col overflow-hidden">
                        <div class="flex items-center border-b border-[var(--border-color)] px-3 py-2"><span class="font-medium text-[var(--text-primary)]"><span class="mr-2" aria-hidden="true">"◉"</span>{move || t!(i18n, common_preview)}</span><button type="button" class=format!("{} ml-auto", button) disabled=move || state.output.get().is_empty() on:click=on_copy title="Copy formatted JSON" aria-label="Copy formatted JSON"><span aria-hidden="true">"⧉"</span><span class="ml-1 hidden md:inline" aria-live="polite">{move || if state.copied.get() { {move || t!(i18n, common_copied)} } else { {move || t!(i18n, common_copy)} }}</span></button></div>
