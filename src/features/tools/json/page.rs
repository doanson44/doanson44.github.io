        ...
        <main class="flex flex-1 flex-col overflow-hidden">
            <div class="flex flex-nowrap items-center gap-1 border-b border-[var(--border-color)] p-2" id="json-toolbar">
                <div class="ml-auto flex flex-nowrap gap-1">
                    ...
                    <button type="button" class=button title="Clear JSON" on:click=move |_| state.clear()><span aria-hidden="true">"×"</span><span class="ml-1 hidden lg:inline">{move || t_string!(i18n, common_clear)}</span></button>
                </div>
            </div>
